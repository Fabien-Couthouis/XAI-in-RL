...
class CustomBuild(build_ext):
    ...
    def run(self):
        ...
        try:
            use_prebuilt_library = int(os.environ.get(
                "GFOOTBALL_USE_PREBUILT_SO", "0"))
        except ValueError:
        ...
        if use_prebuilt_library:
            if os.system("cp third_party/gfootball_engine/lib/prebuilt_gameplayfootball.so " + dest_dir + "/_gameplayfootball.so"):
                raise OSError(
                    "Failed to copy pre-built library to a final destination %s." % dest_dir)
        else:
