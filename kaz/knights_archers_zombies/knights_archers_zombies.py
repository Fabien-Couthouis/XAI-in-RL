...
class raw_env(AECEnv, EzPickle):
    ...
    def __init__(self, spawn_rate=20, num_archers=2, num_knights=2, killable_knights=True, killable_archers=True, pad_observation=True, black_death=True, line_death=False, max_frames=900, archers=None, knights=None, zombies_lives=1):
        EzPickle.__init__(self, spawn_rate, num_archers, num_knights, killable_knights,
                          killable_archers, pad_observation, black_death, line_death, max_frames, archers, knights, zombies_lives)
        assert num_archers >= 0
        assert num_knights >= 0
        # Game Constants
        ...
        archers = archers or [Archer(agent_name="archer_" + str(i))
                              for i in range(num_archers)]
        knights = knights or [Knight(agent_name="knight_" + str(i))
                              for i in range(num_knights)]

        assert len(archers) == num_archers
        for i in range(num_archers):
            archer = archers[i]

            self.archer_dict["archer{0}".format(
                self.archer_player_num)] = archer
            self.archer_dict["archer{0}".format(
                self.archer_player_num)].offset(i * 50, 0)
            self.archer_list.add(
                self.archer_dict["archer{0}".format(self.archer_player_num)])
        ...
        assert len(knights) == num_knights
        for i in range(num_knights):
            knight = knights[i]

            self.knight_dict["knight{0}".format(
                self.knight_player_num)] = knight
            self.knight_dict["knight{0}".format(
                self.knight_player_num)].offset(i * 50, 0)
            self.knight_list.add(
                self.knight_dict["knight{0}".format(self.knight_player_num)])
            self.all_sprites.add(
                self.knight_dict["knight{0}".format(self.knight_player_num)])
            self.agent_list.append(
                self.knight_dict["knight{0}".format(self.knight_player_num)])
        ...
        self.reinit()
        self.zombies_lives = zombies_lives
                ...
                if not self.sword_list:      # Sword List is Empty
                    if not self.knight_killed:
                        for i in range(0, self.knight_player_num + 1):
                            self.sword_dict['sword{0}'.format(i)] = Sword(
                                (self.knight_dict['knight{0}'.format(i)]))
                            self.sword_list.add(
                                self.sword_dict[('sword{0}'.format(i))])
                            self.all_sprites.add(
                                self.sword_dict[('sword{0}'.format(i))])
                ...
                if not self.archer_killed:
                    for i in range(0, self.archer_player_num + 1):
                        if i == self.agent_index:
                            self.arrow_dict[('arrow{0}'.format(i))] = Arrow(
                                self.archer_dict[('archer{0}'.format(i))])
                            self.arrow_list.add(
                                self.arrow_dict[('arrow{0}'.format(i))])
                            self.all_sprites.add(
                                self.arrow_dict[('arrow{0}'.format(i))])
    ...
    def spawn_zombie(self, zombie_spawn_rate, zombie_list, all_sprites):
        zombie_spawn_rate += 1
        zombie = Zombie(self.np_random, health=self.zombies_lives)
    ...
    def zombie_arrow(self, zombie_list, arrow_list, all_sprites, score):
        for arrow in arrow_list:
            ...
            for zombie in zombie_arrow_list:
                ...
                zombie.remove_lives(arrow.power)
                if not zombie.is_alive():
                    zombie_list.remove(zombie)
                    all_sprites.remove(zombie)
                    # score += 1
    ...
    def observe(self, agent):
        ...
        if not agent_obj.alive:
            ...
            cropped[startx:endx, starty:endy,
                    :] = screen[lower_x_bound:upper_x_bound, lower_y_bound:upper_y_bound, :]
    ...
    def step(self, action, observe=True):
        ...
        if self._agent_selector.is_last():
            # Controls the Spawn Rate of Weapons
            self.sword_spawn_rate, self.arrow_spawn_rate = self.check_weapon_spawn(
                self.sword_spawn_rate, self.arrow_spawn_rate)
        ...
        sp = self.spawnPlayers(action, self.knight_player_num, self.archer_player_num, self.knight_list,
                               self.archer_list, self.all_sprites, self.knight_dict, self.archer_dict)
        # Knight
        ...
        # Spawn Weapons
        sw = self.spawnWeapons(action, self.agent_name_mapping[agent], self.agent_list, self.sword_spawn_rate, self.arrow_spawn_rate, self.knight_killed, self.archer_killed, self.knight_dict,
                               self.archer_dict, self.knight_list, self.archer_list, self.knight_player_num, self.archer_player_num, self.all_sprites, self.sword_dict, self.arrow_dict, self.sword_list, self.arrow_list)
        # Sword
        ...
        if self._agent_selector.is_last():
            ...
            # Spawning Zombies at Random Location at every 100 iterations
            self.zombie_spawn_rate, self.zombie_list, self.all_sprites = self.spawn_zombie(
            ...
            # Zombie Kills the Arrow
            self.zombie_list, self.arrow_list, self.all_sprites, self.score = self.zombie_arrow(
                self.zombie_list, self.arrow_list, self.all_sprites, self.score)

            # Zombie Kills the Sword
            self.zombie_list, self.sword_list, self.all_sprites, self.score = self.zombie_sword(
                self.zombie_list, self.sword_list, self.all_sprites, self.score)

            # Zombie Kills the Archer
            if self.killable_archers:
                self.zombie_archer(
                    self.zombie_list, self.archer_list, self.all_sprites, self.archer_killed)

            # Zombie Kills the Knight
            if self.killable_knights:
                self.zombie_list, self.knight_list, self.all_sprites, self.knight_killed, self.sword_list, self.sword_killed = self.zombie_knight(
                    self.zombie_list, self.knight_list, self.all_sprites, self.knight_killed, self.sword_list, self.sword_killed)

            # Kill the Sword when Knight dies
            self.sword_killed, self.sword_list, self.all_sprites = self.kill_sword(
                self.sword_killed, self.sword_list, self.all_sprites)
    ...
    def check_game_end(self):
        ...
        self.run = self.zombie_all_players(
            self.knight_list, self.archer_list, self.run)
