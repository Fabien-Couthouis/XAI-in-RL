...
class PGTester(object):
    ...
    def run_step(self, state, schedule, last_action, last_hidden, info={}):
        state = cuda_wrapper(prep_obs(state).contiguous().view(
            1, self.args.agent_num, self.args.obs_size), cuda=self.cuda_)
        if self.args.model_name in ['schednet']:
            weight = self.behaviour_net.weight_generator(state).detach()
            schedule, _ = self.behaviour_net.weight_based_scheduler(
                weight, exploration=False)
        action_out = self.action_logits(
            state, schedule, last_action, last_hidden, info)
    ...
    def run_game(self, episodes, render):
        action = cuda_wrapper(torch.zeros(
            (1, self.args.agent_num, self.args.action_dim)), cuda=self.cuda_)
        info = {}
        ...
        if self.args.model_name in ['ic3net']:
            gate = self.behaviour_net.gate(
                last_hidden[:, :, :self.args.hid_size])
        ...
        for ep in range(episodes):
            ...
            while True:
                if render:
                    self.env.render()
                state, action, done, reward, success = self.run_step(
                    state, schedule, action, last_hidden, info=info)
    ...
    def print_info(self):
        ...
        print(
            'Mean Reward: {:2.4f}/{:2.4f}'.format(np.mean(self.all_reward), np.std(self.all_reward)))
        print('Mean Success: {:2.4f}/{:2.4f}'.format(
            np.mean(self.all_success), np.std(self.all_success)))
        print(
            'Mean Turn: {:2.4f}/{:2.4f}'.format(np.mean(self.all_turn), np.std(self.all_turn)))
