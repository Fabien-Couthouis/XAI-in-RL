        ...
        if status is 'train':
            if exploration:
                if args.epsilon_softmax:
                    ...
                    p_a = (1 - eps) * torch.softmax(logits,
                                                    dim=-1) + eps / logits.size(-1)
...
def translate_action(args, action, env):
    if not args.continuous:
        actual = [act.detach().squeeze().cpu().numpy()
                  for act in torch.unbind(action, 1)]
...
def cuda_wrapper(tensor, cuda):
    if isinstance(tensor, torch.Tensor):
        ...
        raise RuntimeError(
            'Please enter a pytorch tensor, now a {} is received.'.format(type(tensor)))
...
def unpack_data(args, batch):
    ...
    last_step = cuda_wrapper(torch.tensor(
        batch.last_step, dtype=torch.float).contiguous().view(-1, 1), cuda)
    done = cuda_wrapper(torch.tensor(
        batch.done, dtype=torch.float).contiguous().view(-1, 1), cuda)
    actions = cuda_wrapper(torch.tensor(
        np.stack(list(zip(*batch.action))[0], axis=0), dtype=torch.float), cuda)
    last_actions = cuda_wrapper(torch.tensor(
        np.stack(list(zip(*batch.last_action))[0], axis=0), dtype=torch.float), cuda)
